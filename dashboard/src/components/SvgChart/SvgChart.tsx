...
export function SvgChart({ hum, dist }: Props) {
  ...
  // Last known values for live labels
    ...
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-4 text-xs text-zinc-500">
          ...
          <span className="flex items-center gap-1.5">
            <span className="w-5 border-t border-dashed border-blue-500/40 inline-block" />
            Umbral niebla (80%)
          </span>
        </div>
      </div>
        ...
        <svg width={width} height={H} className="block overflow-visible">
          <g transform={`translate(${PAD.left},${PAD.top})`}>
            ...
            {yTicks.map(v => {
              ...
              return (
                <g key={v}>
                  <line x1={0} y1={y} x2={IW} y2={y} stroke="#27272a" strokeWidth="1" />
                  <text x={-8} y={y + 4} textAnchor="end" fontSize="10" fill="#52525b"
            ...
            {/* Fog threshold dashed line */}
            <line
              x1={0} y1={IH * 0.2} x2={IW} y2={IH * 0.2}
              stroke="#3b82f6" strokeWidth="1" strokeDasharray="5,4" opacity="0.35"
            />

            {/* Distance line (normalized ÷4) */}
            {distPath && (
              <path
                d={distPath}
                fill="none"
            ...
            {/* Axes */}
            <line x1={0} y1={0}  x2={0}  y2={IH} stroke="#3f3f46" strokeWidth="1" />
            <line x1={0} y1={IH} x2={IW} y2={IH} stroke="#3f3f46" strokeWidth="1" />
